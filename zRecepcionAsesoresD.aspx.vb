...
Partial Public Class zRecepcionAsesoresD
    ...
    Protected Sub rdlLavado_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Else
            sqry = " INSERT into tb_citas (idAsesor, idTecnico, fecha, noPlacas, Cliente, Vehiculo, Color, Ano, Cilindros, Servicio, tipoCliente, noOrden, horaAsesor, conCita, enUso, idServicio, vin, servicioCapturado, CarryOver, Status, vhRecepcion, vhInventario, numcita, colorPrisma, observaciones, id_hd, horarampa) "
            sqry += " select top 1 idAsesor, null, (select getdate()), noPlacas, Cliente, Vehiculo, Color, Ano, Cilindros, 60, tipoCliente, noOrden, horaAsesor, case when NUMCITA<>'0' then 1   else 0 end, 0, '151', vin, 'Lavado', null, 'EN LAVADO', 0, 0, numcita, colorPrisma, comentariosLavado , id_hd, left(convert(varchar, getdate(), 108), 5)"
    ...
    Sub LlenadgLibres(sfecha As String)
        ...
        Try
            sqry += " Select * "
            sqry += " from " & System.Web.Configuration.WebConfigurationManager.AppSettings("v_zrececpcionDLibres") & "  where convert(varchar,fecha,12)=convert(varchar,cast('" & String.Format("{0:s}", CDate(sfecha)) & "' as datetime),12)"
    ...
    Sub LlenadgPrincipal(sFecha As String)
        ...
        Try
            ...
            sqry += " Select * "
            sqry += " from " & System.Web.Configuration.WebConfigurationManager.AppSettings("v_zrececpcionDCC") & " where convert(varchar,fecha,12)=convert(varchar,cast('" & String.Format("{0:s}", CDate(sFecha)) & "' as datetime),12)"
    ...
    Protected Sub chkInicio_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If CType(sender, CheckBox).Checked = True Then
            ...
            Session("noPlacas") = CType(dg1.Items(sender.parent.parent.itemindex).FindControl("lnknoPlacas"), LinkButton).Text
        Else
    ...
    Protected Sub chkSCInicio_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If CType(sender, CheckBox).Checked = True Then
            ...
            If dgLibres.Items(sender.parent.parent.itemindex).Cells(1).Text <> "&nbsp;" And Len(dgLibres.Items(sender.parent.parent.itemindex).Cells(1).Text.Trim) > 0 Then
                ...
                Session("noPlacas") = CType(dgLibres.Items(sender.parent.parent.itemindex).FindControl("lnknoPlacas"), LinkButton).Text
    ...
    Protected Sub chkSCFin_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If CType(sender, CheckBox).Checked = True Then
            If CType(sender, CheckBox).Checked Then
                ...
                sqry += "	on d.noPlacas collate Latin1_General_CI_AS=b.noPlacas"
