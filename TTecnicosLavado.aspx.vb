...
Partial Public Class TTecnicosLavado
    ...
    Sub iniciaTablero2()
        ...
        Session("dtPosYL") = dtPos


        Session.Remove("ObtenAusenciasL")
    End Sub
        ...
        For i = 1 To OBJCOL.Count
            ...
            Try
                If objchip.STATUS.ToLower = "detenida" Or objchip.STATUS.ToLower = "iniciada" Or objchip.STATUS.ToLower = "reiniciada" Then
                    ...
                    sstop = CType(Session("dtPosYL"), DataTable).Select("OrdenP='" & objchip.NOORDEN & "' and IdTecnico='" & objchip.IDTECNICO & "'")(0)("posYP") + 30 & "px"
                Else
                    iNivel = 1
                    sstop = CType(Session("dtPosYL"), DataTable).Select("OrdenP='" & objchip.NOORDEN & "' and IdTecnico='" & objchip.IDTECNICO & "'")(0)("posYP") & "px"
                End If
            ...
            If objchip.STATUSOS <> "SSINCARGO" And objchip.STATUSOS <> "Facturado" And objchip.STATUSOS <> "Remisionado" Then
                Try
                    'chipBoard(i, oChip, objchip.IDCHIP, CInt((objchip.SERVICIO / 60) * 9.437), objchip.NOORDEN, ((CDate(objchip.FECHA).DayOfYear - (Date.Now.AddMonths(-1).DayOfYear)) * 75.5) + IIf(((CDate(objchip.HORARAMPA).Hour - 9 - 1) * 9.437) > 0, ((CDate(objchip.HORARAMPA).Hour - 9 - 1) * 9.437), 0) + 92 & "px", CType(Session("dtPosYL"), DataTable).Select("OrdenP='" & objchip.NOORDEN & "' and IdTecnico='" & objchip.IDTECNICO & "'")(0)("posYP") & "px", sColor, sColorServicio, objchip.ID, objchip.NOPLACAS, objchip.SERVICIO, objchip.OBSERVACIONES)
                    chipBoard(i, oChip, objchip.IDCHIP, CInt((objchip.SERVICIO / 60) * (ifac / inumHoras)), objchip.VEHICULO, ((CDate(objchip.FECHA).DayOfYear - (CDate(lblCalendar.Text).AddDays(-inumLeft).DayOfYear)) * ifac) + IIf(((CDate(objchip.HORARAMPA).Hour * 60) - (iHorai * 60)) > 0, ((((CDate(objchip.HORARAMPA).Hour * 60) - (iHorai * 60)) + CDate(objchip.HORARAMPA).Minute) * ((ifac / inumHoras) / 60)), 0) + 92 & "px", sstop, sColor, sColorServicio, objchip.ID, objchip.SERVICIOCAPTURADO, objchip.SERVICIO, objchip.OBSERVACIONES, bAlarm, objchip)
    ...
    Private Sub whuxgaPT_PreRenderComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRenderComplete
        Dim OBJARRAY As ArrayList = CType(Session("imgLstL"), ArrayList)
