...
Partial Public Class TCitas2
    ...
    Dim ifac As Double = (60) * inumHoras
    Dim sMensajes As ArrayList
    ' Dim cNivelLavado As Integer = 2
    ...
    Sub iniciaTablero2()
        ...
        Else
            dt = TablerosUtilsHyP.ObtenTecnicosMasVehiculosCitas2(Request.Url.Segments(Request.Url.Segments.Length - 1), CDate(lblCalendar.Text))
        End If


        Dim dt0 As DataView = TablerosUtilsHyP.ObtenCitasmasAsesoresmasLavado.DefaultView
    ...
    Sub iniciaTablero(ByVal bTecnicos As Boolean, Optional sVehiculo As String = "", Optional bDetenidos As Boolean = False)
        ...
        Dim iNumbahias As Integer, iNumbahias2 As Integer, iNumbahias3 As Integer
        Dim ilargo As Integer = 0, iLargo2 As Integer = 0, ilargo3 As Integer = 0
        Dim iwidth As Integer = "7500"
        Dim dt As DataView = TablerosUtilsHyP.ObtenCitasmasAsesoresmasLavado.DefaultView
        ...
        iLargo2 = ((iNumbahias2) * 30)
        ilargo3 = ((iNumbahias3) * 30)
        If bTecnicos Then
            ...
            sScript += "lx = ((hours * 60) + minutes);(lx *(60/2));lx=lx+ifac;" & vbCrLf
            ...
            sScript += " for(var d_i = 1; d_i <= " & iNumbahias3 & "; d_i++){ " & vbCrLf
            sScript += "    ejeY += 30; " & vbCrLf
            sScript += "    jg_doc.setColor('#ECEDD8');jg_doc.fillRect(0, ejeY-30, " & iwidth + 92 & ", 30); " & vbCrLf

            sScript += "    if (d_i == " & iNumbahias3 & " || d_i == " & iNumbahias3 * 2 & "  || d_i == " & iNumbahias3 * 3 & "  || d_i == " & iNumbahias3 * 4 & " || d_i == " & iNumbahias3 * 5 & "){" & vbCrLf
            sScript += "        jg_doc.setColor('Black');   jg_doc.fillRect(0, ejeY, " & iwidth + 92 & ", 2);} " & vbCrLf
            sScript += "    else{jg_doc.setColor('Darkslategray');jg_doc.drawLine(0, ejeY, " & iwidth + 92 & ", ejeY);} " & vbCrLf
            sScript += " }" & vbCrLf
            ...
            sScript += "    ejeY += 30; " & vbCrLf
            sScript += "    jg_doc.setColor('#D3E1E6');jg_doc.fillRect(0, ejeY-30, " & iwidth + 92 & ", 30); " & vbCrLf
            ...
            sScript += " for(var d_d = 1; d_d < 13; d_d++){ posX3=posX3+5;" & vbCrLf
            ...
            sScript += "            for(var d_j = 1; d_j < " & inumHoras + 1 & "; d_j++){" & vbCrLf
            sScript += "  for(var d_d = 1; d_d < 13; d_d++){ posX3=posX3+5;" & vbCrLf
            sScript += "jg_doc.setColor('" & SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "bTableroGrid") & "'); jg_doc.drawLine(posX3, " & ilargo & ", posX3, " & ilargo + ilargo3 & ");" & vbCrLf
            sScript += "               }" & vbCrLf

            sScript += "                jg_doc.setColor('" & SCCParametros.ObtenParametros2(Request.Url.Segments(Request.Url.Segments.Length - 1), "bTableroGrid") & "');   jg_doc.drawLine(posX2,  " & ilargo & ", posX2, " & ilargo + ilargo3 & ");" & vbCrLf
            sScript += "                jg_doc.setColor(' " & SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "bTableroGrid2") & "'); jg_doc.drawLine(posX2+(60/2),  " & ilargo & ", posX2+(60/2), " & ilargo + ilargo3 & ");" & vbCrLf
            sScript += "                posX2=posX2+60;}" & vbCrLf
            sScript += "        jg_doc.setColor('Navy'); jg_doc.drawLine(posX,  " & ilargo & ", posX, " & ilargo + ilargo3 & "); " & vbCrLf
            sScript += "     posX = posX+" & ifac & ";}" & vbCrLf
            sScript += " " & vbCrLf
            ...
            sScript += "            for(var d_j = 1; d_j < " & inumHoras + 1 & "; d_j++){" & vbCrLf
            sScript += " for(var d_d = 1; d_d < 13; d_d++){ posX3=posX3+5;" & vbCrLf
            sScript += "jg_doc.setColor('" & SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "bTableroGrid") & "'); jg_doc.drawLine(posX3, " & ilargo & ", posX3, " & ilargo + iLargo2 + ilargo3 & ");" & vbCrLf
            sScript += "               }" & vbCrLf

            sScript += "                jg_doc.setColor('" & SCCParametros.ObtenParametros2(Request.Url.Segments(Request.Url.Segments.Length - 1), "bTableroGrid") & "');  jg_doc.drawLine(posX2,  " & ilargo & ", posX2, " & ilargo + iLargo2 + ilargo3 & ");" & vbCrLf
            sScript += "                jg_doc.setColor(' " & SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "bTableroGrid2") & "'); jg_doc.drawLine(posX2+(60/2),  " & ilargo & ", posX2+(60/2), " & ilargo + iLargo2 + ilargo3 & ");" & vbCrLf
            sScript += "                posX2=posX2+60;}" & vbCrLf
            sScript += "        jg_doc.setColor('Navy'); jg_doc.drawLine(posX,  " & ilargo & ", posX, " & ilargo + iLargo2 + ilargo3 & "); " & vbCrLf
            sScript += "     posX = posX+" & ifac & ";}" & vbCrLf
            sScript += " " & vbCrLf
            ...
            For i = 1 To inumDias
                ...
                txtDia2.Style.Value = "border:none;position:absolute;left:" & 92 + ((ifac) * (i - 1)) & "px;top:50px;  "

                txtDia2.InnerText = ddate.AddDays(i - 1).ToShortDateString

                dtPosDias.Rows.Add(New Object() {CDate(txtDia2.InnerText), 92 + ((ifac) * (i - 1))})
                txtDia3.Style.Value = "width:" & ifac & "px;height:10px; Font-weight:bold;font-family:Arial;font-size:12px;"
                txtDia3.InnerHtml = "<table border='0' cellspacing=0 cellpadding=0   ><tr>"
                For ii = 0 To inumHoras - 1
                    txtDia3.InnerHtml = txtDia3.InnerHtml & "<td  style='border:solid .01em black; width:" & Math.Round(ifac / inumHoras) & "px; text-align:center;background-color:white; ' >" & String.Format("{00:d}", ii + iHorai) & ":00</td>"
        ...
        For i = 1 To OBJCOL.Count
            ...
            txtMec.Style.Value = "position:absolute;top:" & ((Fix((i - 1) / III)) * 32) + 75 & "px;left:" & xleft + 5 & "px;width:100px;background-color:" & "#fff" & ";Font-weight:bold;font-family:  Lucida Grande,Lucida Sans,Arial,sans-serif;font-size:10px;border:solid 1px black;"
            ...
            AgregaInfo(txtMec.ID, objchip.ID & "___" & objchip.NUMCITA & "___" & objchip.NOPLACAS & "___" & objchip.IDSERVICIO & "___" & objchip.COLORPRISMA & "___" & objchip.CLIENTE & "___" & objchip.ASYSRENGLON & "___" & objchip.SERVICIOCAPTURADO & "___" & objchip.HORAASESOR & "___" & objchip.VEHICULO & "___" & objchip.VIN & "___" & objchip.AÑO & "___" & objchip.COLOR & "___" & objchip.IDASESOR & "___" & objchip.TELEFONOS & "___" & objchip.SERVICIO & "___" & objchip.IDITEM & "___" & objchip.NOPRISMA & "___" & objchip.TIPOCLIENTE & "___" & objchip.FECHA.ToShortDateString & "___" & objchip.HORAASESOR & "___" & objchip.IDBLINSUR)
    ...
    Sub PINTACHIPS()
        ...
        Else
            OBJCOL = OBJCOL.ObtenChipsAllCitas2P(Request.Url.Segments(Request.Url.Segments.Length - 1), CDate(lblCalendar.Text), False)
        ...
        For i = 1 To OBJCOL.Count
            ...
            If Not Session("buscaChip") Is Nothing Then
                ...
                End Select
            End If
            If CDate(objchip.FECHA.ToShortDateString & " " & objchip.HORARAMPA) <= CDate(lblCalendar.Text) And objchip.STATUS.ToLower = "pendiente" Then
                bAlarm = True
            Else
                bAlarm = False
            End If
    ...
    Sub displayChip(ByVal iItem As Integer, ByVal sDiv As String, ByVal sL As String, ByVal sT As String, ByVal sWidth As String, ByVal sImageChip As String, ByVal sColor As String, ByVal scolorservicio As String, ByVal iTime As Integer, ByVal lblChp1 As String, ByVal lblChp2 As String, ByVal lblChp3 As String, ByVal lblChp4 As String, ByVal sServiciocapturado As String, ByVal sServicio As String, ByVal sObservaciones As String, ByVal bAlarm As Boolean, ByVal objChip As ChipHYP)
        ...
        If objChip.SERVICIOCAPTURADO.ToLower = "lavado" Then
            ixdiv.Attributes.Add("class", "menu4")
        ElseIf objChip.TIPOCLIENTE.ToLower = "entrega" Or objChip.TIPOCLIENTE.ToLower = "recepcion" Then
            ixdiv.Attributes.Add("class", "menu3")
        ...
        If objChip.CONCITA = True Then
            ixdiv.Attributes.Add("title", "--CITA " & objChip.NUMCITA & "-- HoraCita " & objChip.HORAASESOR & vbCrLf & "Vehiculo: " & objChip.VEHICULO & " " & objChip.COLOR & vbCrLf & " Placas: " & objChip.NOPLACAS & vbCrLf & " Torre: " & objChip.COLORPRISMA & vbCrLf & "VIN: " & objChip.VIN & vbCrLf & "Servicio: " & sServiciocapturado & vbCrLf & "Duracion: " & System.Math.Round((sServicio / 60), 2) & " horas" & vbCrLf & "Observaciones: " & sObservaciones & "" & vbCrLf & "Estatus: " & objChip.STATUS.ToUpper & "" & vbCrLf & sComentarios)
    ...
    Sub _modtiempo(sdhtml As String, im As Integer)
        ...
        iTiempo = System.Math.Round(Decimal.Round(CDec(iTiempo / 60), 2) * 60, MidpointRounding.AwayFromZero)
    ...
    Function Validar(ByVal obj As ChipHYP, ByVal iOperacion As String) As Boolean
        ...
        Select Case iOperacion
            Case "G"
                ...
                If DateDiff(DateInterval.Day, obj.FECHA, obj.FECHAORIGINAL) <> 0 And SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "iValidacionDiaCita") = "1" Then
                    sMensajes.Add("No se puede grabar la cita a un dia distino de " & obj.FECHAORIGINAL.ToShortDateString)
                ...
                Try
                    ...
                    End If
                Catch ex As Exception
                    sMensajes.Add("hora cita no valida")
        ...
        For Each iobj As ChipHYP In OBJCOL
            If iobj.ID <> obj.ID And TablerosUtilsHyP.ObtenIdTecnicosCitas(obj.TOPPX).ToLower = iobj.IDTECNICO.ToLower And obj.FECHA.DayOfWeek = iobj.FECHA.DayOfWeek And iobj.STATUS = "Pendiente" Then
                ...
                Select Case iobj.TIPOCLIENTE.ToLower
                    Case "entrega", "recepcion", "lavado"

                        If iobj.TIPOCLIENTE = obj.TIPOCLIENTE Then
                            If TablerosUtilsHyP.ObtenPosXxHora(obj.HORARAMPA) >= TablerosUtilsHyP.ObtenPosXxHora(iobj.HORARAMPA) And TablerosUtilsHyP.ObtenPosXxHora(obj.HORARAMPA) < (TablerosUtilsHyP.ObtenPosXxHora(iobj.HORARAMPA) + iwidthServ) Then
                                sMensajes.Add("El chip se encimaria")
                    ...
                    Case Else
                        If obj.TIPOCLIENTE.ToLower.Trim <> "recepcion" And obj.TIPOCLIENTE.ToLower.Trim <> "entrega" And obj.TIPOCLIENTE.ToLower.Trim <> "lavado" Then
                            If TablerosUtilsHyP.ObtenPosXxHora(obj.HORARAMPA) >= TablerosUtilsHyP.ObtenPosXxHora(iobj.HORARAMPA) And TablerosUtilsHyP.ObtenPosXxHora(obj.HORARAMPA) < (TablerosUtilsHyP.ObtenPosXxHora(iobj.HORARAMPA) + iwidthServ) Then
                                sMensajes.Add("El chip se encimaria")
                                '  TablerosUtilsHyP.iMsgBox(Me.Page, "El chip se encimaria")
                                Return False
    ...
    Sub _grabarchip(sdhtml As String, im As Integer)
        ...
        objChip.NOMBREDIA = CDate(lblCalendar.Text).DayOfWeek
        ...
        If Validar(objChip, "G") Then
            ...
            If sultimochip Then
                sMensajes.Add("La cita " & objChip.NUMCITA & " se agendo el " & objChip.FECHA.ToShortDateString & " a las " & objChip.HORAASESOR & " con un costo de " & sscosto & " y con el asesor " & TablerosUtilsHyP.ObtenNombreAsesor(objChip.IDASESOR))
                '  TablerosUtilsHyP.iMsgBox(Me.Page, )
    ...
    Protected Sub imgBtnUpd_DMS_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpd_DMS.Click
        ...
        Else
            For i As Integer = 0 To Me.iMulti.Value.Split(",").Length() - 1
                ...
                schip = iMulti.Value.Split(",")(i)
                _grabarchip(schip, Me.iMulti.Value.Split(",").Length())
            Next
    ...
    Sub llenaindicadores()
        ...
        Dim iFecha As Date = CDate(lblCalendar.Text)
        ...
        Do While iFechaIni <= iFechaFin
            i = i + 1
            y = (From p As ChipHYP In objCHIPCol Where p.FECHA.ToShortDateString = iFechaIni.ToShortDateString And p.IDTECNICO <> "" And p.TIPOCLIENTE.ToString().Trim() <> "lavado" Select p.IDTECNICO).Distinct()

            For Each c In y
                x = (From p As ChipHYP In objCHIPCol Where p.FECHA.ToShortDateString = iFechaIni.ToShortDateString And p.IDTECNICO = c.ToString() And p.NUMCITA <> 0 And p.TIPOCLIENTE.ToString().Trim() <> "lavado" Group p By p.IDTECNICO Into Group Select total = Group.Sum(Function(p) (p.SERVICIO))).FirstOrDefault()
                w = (From p As ChipHYP In objCHIPCol Where p.FECHA.ToShortDateString = iFechaIni.ToShortDateString And p.IDTECNICO = c.ToString() And p.NUMCITA = 0 And p.TIPOCLIENTE.ToString().Trim() <> "lavado" Group p By p.IDTECNICO Into Group Select total = Group.Sum(Function(p) (p.SERVICIO))).FirstOrDefault()
                z = (From p As ChipHYP In objCHIPCol Where p.FECHA.ToShortDateString = iFechaIni.ToShortDateString And p.IDTECNICO = c.ToString() And p.TIPOCLIENTE.ToString().Trim() <> "lavado" Group p By p.IDTECNICO Into Group Select total = Group.Sum(Function(p) (p.SERVICIO))).FirstOrDefault()
