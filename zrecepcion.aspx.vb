...
Partial Class zRecepcion
	...
	Protected Sub imgDelete_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
		...
		lblInf2.Text = dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(16).Text 'id_hd
	...
	Protected Sub dg1_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles dg1.PreRender
		...
		For i = 0 To dg1.Items.Count - 1
			...
			End If
			'exress
			If dg1.Items(i).Cells(12).Text.Trim().ToLower() = "express" Then
				dg1.Items(i).Style.Remove("background-color")
				dg1.Items(i).Style.Add("background-color", "gold")
			End If
	...
	Sub Insertar()
		...
		If Not Valuarcamiones() Then
			If Not (Validaciones.validaAnteriores(txtPlacas.Text.Trim)) Then
			...
			Else
				Dim m As String = "El Vehiculo no puede ser ingresado porque cuenta con un registro pendiente por darle salida"
				Response.Write("<script>window.alert('" & m & "');</script>")
			End If
		Else
			If Not (Validaciones.validaAnteriores(txtPlacas.Text.Trim)) Then
				sqry = "Insert into " & System.Web.Configuration.WebConfigurationManager.AppSettings("dmsCitasHD") & " ( numcita,noorden,idasesor,NOPLACAS, FECHA,CLIENTE, vehiculo, COLOR,horallegada, colorprisma,ano, cilindros, kilometraje,idop, bahia, ContactoTelefono) values('0','0','" & cboAsesor.SelectedItem.Value & "','" & txtPlacas.Text.Trim.ToUpper & "',cast('" & String.Format("{0:s}", CDate(Now.ToShortDateString)) & "' as datetime), '', '" & cboVehiculo.SelectedItem.Text & "',  '" & cboColor.SelectedItem.Text & "', getdate(),'" & txtTorre2.Text.Trim.ToUpper & "',0,0,0,0,0,'" & Me.cmbTipoUnidad.SelectedItem.Text & "')"
			Else
				Dim m As String = "El Vehiculo no puede ser ingresado porque cuenta con un registro pendiente por darle salida"
				Response.Write("<script>window.alert('" & m & "');</script>")
			End If
	...
	Protected Sub chkRecibido_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
		...
		If CType(sender, CheckBox).Checked Then
			...
			If lblInf2.Text = "0" And dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(14).Text.Trim <> "0" Then
				lblInf3.Text = dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(14).Text.Trim 'numcita
				lblOp.Text = "01"
			End If
			If lblInf2.Text = "0" And dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(19).Text.Trim <> "0" Then
				lblInf3.Text = dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(19).Text.Trim 'noorden
				lblOp.Text = "02"
			End If
	...
	Sub guardarLlegada2(ByVal id_hd As String, snumcita As String)
		...
		Dim dt As New DataTable, sidhd As String, snoplacas As String
	...
	Sub guardarTorre()
		...
		sqry = " Update  "
		sqry += "   " & System.Web.Configuration.WebConfigurationManager.AppSettings("dmsCitasHD") & "  set kilometraje='" & txtTorre.Text.Trim & "'"
		sqry += " where  id_hd =" & lblInf2.Text & " "
	...
	Protected Sub LinkButton1_Click2(ByVal sender As Object, ByVal e As EventArgs)
		...
		lblOp.Text = 11
		lblInf1.Text = dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(15).Text 'placas
		lblInf2.Text = dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(16).Text 'id_hd
		lblInf3.Text = dg1.Items(CType(sender.parent.parent, DataGridItem).ItemIndex).Cells(2).Text.Trim 'cliente
		lblMSG.Text = ""
	...
	Private Function Valuarcamiones() As Boolean
		If Me.cmbTipoUnidad.Visible = True Then
			Return True
		Else
			Return False
		End If
