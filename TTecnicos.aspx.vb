...
Partial Public Class TTecnicos
    ...
    Dim ifac As Double = (59.399999999999999) * inumHoras
    ...
    Private Sub whuxgaPT_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        ifac = (59.399999999999999) * inumHoras
   ...
   Private Sub AbrirChip(ByVal iId As Int64, ByVal ssTop As String, ByVal ssLeft As String)
    ...
    Sub displayChip(ByVal iItem As Integer, ByVal sDiv As String, ByVal sL As String, ByVal sT As String, ByVal sWidth As String, ByVal sImageChip As String, ByVal sColor As String, ByVal scolorservicio As String, ByVal iTime As Integer, ByVal lblChp1 As String, ByVal lblChp2 As String, ByVal lblChp3 As String, ByVal lblChp4 As String, ByVal sServiciocapturado As String, ByVal sServicio As String, ByVal sObservaciones As String, ByVal bAlarm As Boolean, ByVal objChip As ChipHYP, ByVal bAlarmF As Boolean)
        ...
        If objChip.CONCITA = True Then
			ixdiv.Attributes.Add("title", "--CITA " & objChip.NUMCITA & "-- HoraCita " & objChip.HORAASESOR & vbCrLf & "Orden:" & objChip.NOORDEN & vbCrLf & "Vehiculo: " & objChip.VEHICULO & " " & objChip.COLOR & vbCrLf & " Placas: " & objChip.NOPLACAS & vbCrLf & " Cliente: " & objChip.CLIENTE & " N° Economico: " & objChip.COLORPRISMA & vbCrLf & "VIN: " & objChip.VIN & vbCrLf & "Servicio: " & sServiciocapturado & vbCrLf & "Duracion: " & System.Math.Round((sServicio / 60), 2) & " horas" & vbCrLf & "Observaciones: " & sObservaciones & "" & vbCrLf & "Estatus: " & objChip.STATUS.ToUpper & "" & vbCrLf & sComentarios)
    ...
    Private Sub whuxgaPT_PreRenderComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRenderComplete
        ...
        sScript += "" & Chr(34) & "pnlDetencion" & Chr(34) & ", "
        ...
        For Each obj As ChipHYP In OBJCOL
            If obj.ID = iId Then
                ...
                If Validar(obj, "I") Then
                    ...
                    ' If (Validaciones.validaConRecepcion(obj.ID)) Then
                    ...
                    'Else
                    'Dim m As String = "El Chip a iniciar no cuenta con un proceso de recepción completo, favor Terminarlo y luego intentarlo nuevamente." & " identificado con:  " & obj.NOPLACAS
        ...
        Select Case sOperacion
            Case "I"
                ...
                If obj.NOORDEN = "0" Then
                    TablerosUtilsHyP.ActualizarOrden(obj, SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "campoenlace"))
                End If
                'If obj.NOORDEN = "0" And obj.TIPOCLIENTE.ToLower() <> "express" Then
                ...
                If SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "iMInicio") <> "1" Then
                    For Each iobj As ChipHYP In OBJCOL
                        If iobj.IDTECNICO = obj.IDTECNICO And obj.FECHA = iobj.FECHA And iobj.ID <> obj.ID Then
                            If iobj.STATUS = "Iniciada" Or iobj.STATUS = "Reiniciada" Then
                ...
                If SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "ChpIniSec") = "1" Then
                    For Each iobj As ChipHYP In OBJCOL
                        If iobj.IDTECNICO = obj.IDTECNICO And obj.FECHA.ToShortDateString = iobj.FECHA.ToShortDateString And iobj.ID <> obj.ID And DateDiff(DateInterval.Minute, CDate(iobj.HORARAMPA), CDate(obj.HORARAMPA)) > 0 Then
                            If iobj.STATUS = "Pendiente" Or iobj.STATUS = "Iniciada" Or iobj.STATUS = "Reiniciada" Then
        ...
        For Each obj As ChipHYP In OBJCOL
            If obj.ID = iId Then
                ...
                If Validar(obj, "F") Then
                    ...
                    obj.VHINVENTARIO = True
                    If rdlLavar.SelectedValue = "1" Then
                        TablerosUtilsHyP.GrabaLavado(obj)
                    Else
                        TablerosUtilsHyP.BorrarLavado(obj)
                    End If
        ...
        For Each obj As ChipHYP In OBJCOL
            If obj.ID = iId Then
                ...
                If Validar(obj, "D") Then
					...
					If Me.cmbtipodetencion.SelectedItem.Value <> 1 Then
						Dim sql As String
						sql = "update tb_citas set hora1 = '" & cmbtipodetencion.SelectedItem.Value & "' where id = '" & obj.ID & "'"
    ...
    Private Sub Timer13_Tick(sender As Object, e As EventArgs) Handles Timer13.Tick
        If SCCParametros.ObtenParametros(Request.Url.Segments(Request.Url.Segments.Length - 1), "logaccion").ToString = "1" Then
            Validaciones.insertaLogs(Session("Usuario"), Request.Url.Segments(Request.Url.Segments.Length - 1), "accion")

        End If
    End Sub

    Private Sub btnFin1_Click(sender As Object, e As ImageClickEventArgs) Handles btnFin1.Click
	...
	Protected Sub btnOK_Click(sender As Object, e As EventArgs)
		...
		Next
		Dim sql As String = ""
		If Me.cmbbahia.SelectedItem.Text = "Express" Then
			sql = "update tb_citas  set seriecolorprisma = 'E--" & Me.cbobahiaExpress.SelectedItem.Text & "--" & Me.cboPuesto.SelectedItem.Text & "' where id = '" & iId & "'"
		Else
			sql = "update tb_citas  set seriecolorprisma = '" & Me.cmbbahia.SelectedItem.Text & "' where id = '" & iId & "'"
		End If
