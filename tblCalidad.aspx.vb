...
Partial Class tblCalidad
    ...
    Sub BuscarFiltro()
        ...
        sQRY = "select a.status estatus_a, (select nombre_empleado from Tb_TECNICOS x where x.id_empleado=a.idtecnico ) as nombretecnico,a.* from Tb_CITAS_RETRABAJOS a WHERE  fecha   between cast('" & String.Format("{0:s}", CDate(txtFec1.Text).AddDays(-1)) & "' as datetime) and cast('" & String.Format("{0:s}", CDate(txtFec2.Text).AddDays(1)) & "' as datetime)  order by a.fecha  desc"
