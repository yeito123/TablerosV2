...
Partial Public Class zcrmCuestionarioDetalleB
    ...
    Sub iniciavalores(ByVal bSnIdEcuesta As String)
        ...
        If Not txtOrden.Text = "" Then
            ...
            sqry = "Select Mail, cp, telefono, tipoauto, direccion, colonia, cliente From bi_capnet..V_DatosCliente where no_orden= '" & Session("Orden") & "'"
            dt = bdclass.SQLtoDataTable(sqry, True)
    ...
    Sub INSERTAR(ByVal sIdPregunta As Integer, ByVal sValor As String)
        ...
        If (Not Session("idencuestaservicio") Is Nothing) And cboEstatusLlamada.SelectedItem.Value.Length > 0 Then
            sqry = "update EncuestaServicioMZD set  EstatusEncuesta=" & Session("idencuestaservicio") & ", Fecha_Estatus=cast('" & String.Format("{0:s}", CDate(txtFechaEstatus.Text)) & "' as datetime), EStatus='" & cboEstatusLlamada.SelectedItem.Text & "' where orden='" & txtOrden.Text.Trim & "' and TCUESTIONARIO=1 and nom_cliente='" & txtCliente.Text.Trim & "'"
    ...
    Protected Sub cmdGuardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdGuardar.Click
        ...
        Dim dt As New DataTable
        Dim sqry As String = ""
        sqry = "SELECT * FROM EncuestaServicioMZD where ORDEN = '" & txtOrden.Text.Trim & "' and TCUESTIONARIO=1  and nom_cliente='" & txtCliente.Text.Trim & "'"
        dt = bdclass.SQLtoDataTable(sqry, True)
        If dt.Rows.Count > 0 Then
            Session("sFechaCaptura") = dt.Rows(0).Item("fecha_captura").ToString()

            sqry = "DELETE EncuestaServicioMZD where ORDEN = '" & txtOrden.Text.Trim & "' and TCUESTIONARIO=1  and nom_cliente='" & txtCliente.Text.Trim & "' "
            bdclass.ExecuteQuery(sqry, True)
        Else
    ...
    Sub PosicionaValoreGuardadosxOrden(ByVal sOrden As String, ByVal scliente As String)
        ...
        sqry = "SELECT * FROM EncuestaServicioMZD where orden = '" & sOrden & "' and TCUESTIONARIO=1 and nom_cliente='" & scliente & "' order by id_pregunta asc"
        dt = bdclass.SQLtoDataTable(sqry, True)
        ...
        For i = 0 To dt.Rows.Count - 1
            ...
            Select Case CInt(dt.Rows(i).Item("id_pregunta").ToString)
                ...
                Case 3
                    If dt.Rows(i).Item("valor").ToString.Trim.ToUpper = "SI" Then
                        OPT3.SelectedIndex = 0
                    Else
                        OPT3.SelectedIndex = 1
                    End If
                Case 4
                    If dt.Rows(i).Item("valor").ToString.Trim.ToUpper = "SI" Then
                        OPT4.SelectedIndex = 0
                    Else
                        OPT4.SelectedIndex = 1
                    End If
                Case 5
                    If dt.Rows(i).Item("valor").ToString.Trim.ToUpper = "SI" Then
                        OPT5.SelectedIndex = 0
                    Else
                        OPT5.SelectedIndex = 1
                    End If
                Case 6
...
Public Class BDClass
    Public Shared sErrLogs As String = System.Web.Configuration.WebConfigurationManager.AppSettings("rDirError").ToString
    Public Shared Sub inErr(ByVal smsg As String, sFileName As String)
        If System.IO.Directory.Exists(sErrLogs) Then
            Try
                System.IO.File.AppendAllText(sErrLogs & sFileName, vbCrLf & smsg & vbCrLf & vbCrLf)
        ...
        End If
    End Sub
    Public Shared ReadOnly Property sqlConnectionStrBI() As String
        Get
            Return ConfigurationManager.ConnectionStrings("biCS").ToString
        End Get
    End Property
    Public Shared ReadOnly Property sqlConnectionStrTablero() As String
        Get
            Return ConfigurationManager.ConnectionStrings("tbCS").ToString
        End Get
    End Property
    Public Shared Function OpenConnection(ByVal BI As Boolean) As SqlConnection
        Dim ObjConnection As SqlConnection
        If BI Then
            ObjConnection = New SqlConnection(sqlConnectionStrBI)
        Else
            ObjConnection = New SqlConnection(sqlConnectionStrTablero)
        End If
    ...
    End Function
    Public Shared Function ExecuteQuery(ByVal SQL As String, ByVal BI As Boolean) As String
        Dim CNN As SqlConnection
        Dim sqry As String = SQL
        CNN = OpenConnection(BI)
        ...
        Catch ex As Exception
            CNN.Close()
            inErr(Now.ToString("s") & ": " & ex.ToString & vbCrLf & SQL & vbCrLf, "Err_exec.txt")
            Return "NO"
        End Try
    End Function
    ...
    Public Shared Function SQLtoDataTable(ByVal SQL As String, ByVal BI As Boolean) As DataTable
        Dim CNN As SqlConnection
        Dim sqry As String = SQL
        CNN = OpenConnection(BI)
        ...
        Catch exx As Exception
            ...
            inErr(exx.ToString & vbCrLf & SQL & vbCrLf, "Err_SQLtoDatatable.txt")
