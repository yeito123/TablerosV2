...
Public Class TblCalidadV2
	...
	Protected Sub btnOK_Click(sender As Object, e As EventArgs)
		...
		Select Case (Session("opc"))
			...
			Case 2
				...
				End If
			Case 3
				sql = "UPDATE TB_CITAS_CALIDAD SET Fecha_hora_ini=GETDATE(), fecha_hora_Fin = GETDATE(), STATUS = 'Finalizada' , comentarios = 'Vehiculo sin Control Calidad' WHERE ID_CALIDAD = '" & num & "'"
				m = BDClass.ExecuteQuery(sql, False)
	...
	Protected Sub llenafinalizados()
		Dim dt As New DataTable
		Dim sQRY As String = ""
		'Me.gridfinalizados = Nothing
		'Me.gridfinalizados.DataBind()
		sQRY = "select a.* from V_CALIDAD_FINALIZADOS a WHERE  fecha   between cast('" & String.Format("{0:s}", CDate(txtFec1.Text).AddDays(0)) & "' as datetime) and cast('" & String.Format("{0:s}", CDate(txtFec2.Text).AddDays(1)) & "' as datetime)  order by a.fecha  desc"
		dt = BDClass.SQLtoDataTable(sQRY, False)
	...
	Protected Sub imgRutaok_Click(sender As Object, e As ImageClickEventArgs)
		Dim selectedindex = CType(sender.Parent.Parent, DataGridItem).ItemIndex
		Me.GridPendientes.SelectedIndex = selectedindex
		Dim num As Integer = CInt(GridPendientes.SelectedItem.Cells(11).Text)
		Dim id As Integer = CInt(GridPendientes.SelectedItem.Cells(0).Text)
		...
		lblidInterno2.Text = id
		lblInicio.Text = inicio
		Dim Sql As String = "UPDATE TB_CITAS_CALIDAD SET Fecha_hora_ini=GETDATE(), fecha_hora_Fin = GETDATE(), STATUS = 'Finalizada' , comentarios = 'Vehiculo sin Control Calidad' WHERE ID_CALIDAD = '" & num & "'"
		m = BDClass.ExecuteQuery(Sql, False)
		llenaPendientes()
		'Session("opc") = 3
		'Me.mpe.Show()
		...
		'lblComentarios.Visible = True
		'lblComentarios.Text = "Esta seguro que desea eliminar el vehículo?"
	End Sub
End Class